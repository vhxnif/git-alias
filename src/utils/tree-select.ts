...
function highlightMatch(text: string, term: string): string {
    ...
    return `${before}${display.highlight.bold(match)}${after}`
}
...
function renderTreeLine<T>(
    ...
    const { yellow, green, overlay0, pink, subtext1 } = color
    ...
    if (searchTerm) {
        ...
        label = isSelected ? pink.bold(rawLabel) : pink(rawLabel)
    } else if (isSelected) {
        label = yellow.bold(rawLabel)
    } else if (isLeaf) {
        label = subtext1(rawLabel)
    } else {
        label = green(rawLabel)
    }

    let displayStr = `${overlay0(indent)}${prefix}${icon}${label}`

    if (isLeaf && node.formatInfo) {
        displayStr += ` ${overlay0('[')}${node.formatInfo}${overlay0(']')}`
    }
...
export default createPrompt<TreeSelectResult<any>, TreeSelectConfig<any>>(
    (config, done) => {
        ...
        const key = (desc: string, k: string) => `${desc}${pink(`<${k}>`)}`

        const pageIndicator =
            displayNodes.length > pageSize
                ? ` ${subtext0(`[${start + 1}-${end}/${displayNodes.length}]`)}`
                : ''

        const searchIndicator = searchMode ? ` ${peach('[SEARCH]')}` : ''
        ...
        if (searchMode) {
            return [
                ...
                ...treeLines,
                '',
                `${key('', 'enter/esc')} ${subtext0('finish search')}`,
            ].join('\n')
        }
        ...
        if (help) {
            helpBar.push('', navHelp, actionHelp)
        }
        return [
