    ...
    if (name && !force) {
      ...
      if (candidates.length > 0) {
        ...
        const scored = candidates.map((branch) => {
          const isRemote = branch.name.startsWith("remotes/") || branch.name.startsWith("origin/")
          const matchResult = calculateBranchMatchScore(name, branch.name, isRemote)
          ...
          const typeWeight = matchResult.type === "exact-word" ? 5 : matchResult.type === "starts-with" ? 3 : 1
          ...
          const totalScore = historyScore * typeWeight * matchResult.distanceScore * branchWeight
        ...
        try {
          ...
          spinner.succeed(`Switched to branch: ${color.mauve.bold(branchName)}`)
          logcmd(result, "git-switch")
          bs.update(branchName, frequency)
          return
        } catch (err: unknown) {
          const msg = (err as ShellError).stderr.toString()
          spinner.fail(`Failed to switch to branch: ${color.mauve.bold(branchName)}`)
